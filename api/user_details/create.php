...
// Include the Transaction model (to validate transaction_id)
...
// Check if the user is logged in and is an admin (only admins should create user details records)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors) && $user_id > 0) {
        try {
            ...
            error_log("API User Details create - User ID validation error: " . $e->getMessage());
        }
    }
    ...
    if (empty($errors) && $card_id > 0) {
        try {
            ...
            error_log("API User Details create - Card ID validation error: " . $e->getMessage());
        }
    }
    ...
    if (empty($errors) && $transaction_id > 0) {
         try {
             ...
             error_log("API User Details create - Transaction ID validation error: " . $e->getMessage());
         }
    }
    ...
    if (empty($errors) && $recharge_id > 0) {
         try {
             ...
             error_log("API User Details create - Recharge ID validation error: " . $e->getMessage());
         }
    }
    ...
    if (empty($errors)) {
        try {
            // Prepare data array for the model to create the user details record
            // Note: Passwords should be pre-hashed before reaching this point.
            $new_user_details_data = [
                ...
                'created_at' => $created_at, // Use provided or current time
                'updated_at' => $updated_at // Use provided or current time
            ];

            // Attempt to create the user details record using the model
            $new_user_details_id = $userDetailsModel->create($new_user_details_data);

            if ($new_user_details_id) {
                $response['success'] = true;
                $response['message'] = 'User details record created successfully!';
                $response['data'] = ['user_details_id' => $new_user_details_id];
            } else {
                $response['message'] = 'Failed to create user details record. Please try again.';
            }
        } catch (PDOException $e) {
            http_response_code(500); // Internal Server Error
            $response['message'] = 'Database error during user details record creation.';
            error_log("API User Details create error: " . $e->getMessage()); // Log the actual error
        }
    } else {
