...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors) && $table_id_to_update > 0) {
        try {
            ...
            error_log("API Waiter update - Existing record fetch error: " . $e->getMessage());
        }
    }
    ...
    if (empty($errors) && $user_id_assigned > 0 && $existing_table_record) {
         try {
             ...
             error_log("API Waiter update - User ID validation error: " . $e->getMessage());
         }
    }
    ...
    if (empty($errors) && $card_id > 0 && $existing_table_record) {
         try {
             ...
             error_log("API Waiter update - Card ID validation error: " . $e->getMessage());
         }
    }
    ...
    if (empty($errors) && $existing_table_record) {
        try {
            ...
            if ($tableOrderModel->update($update_table_data)) {
                ...
                $response['message'] = 'Table record (assignment/order) updated successfully!';
            ...
            error_log("API Waiter update error: " . $e->getMessage()); // Log the actual error
        }
    } else {
