...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors) && $user_id_assigned > 0) {
         try {
             ...
             error_log("API Waiter create - User ID validation error: " . $e->getMessage());
         }
    }
    ...
    if (empty($errors) && $card_id > 0) {
         try {
             ...
             error_log("API Waiter create - Card ID validation error: " . $e->getMessage());
         }
    }
    ...
    if (empty($errors)) {
        try {
            // Prepare data array for the model to create the table record (assignment/order)
            $new_table_data = [
                't_qr' => $t_qr,
                't_name' => $t_name,
                't_s_w' => $t_s_w,
            ...
            if ($new_table_id) {
                ...
                $response['message'] = 'Table record (assignment/order) created successfully!';
            ...
            error_log("API Waiter create error: " . $e->getMessage()); // Log the actual error
        }
    } else {
