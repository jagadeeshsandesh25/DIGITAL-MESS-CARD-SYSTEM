...
// Check if the user is logged in and is an admin (only admins should create cards)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $c_status = $_POST['c_status'] ?? 'Inactive'; // Default to Inactive
    $user_id = (int)($_POST['user_id'] ?? 0);
    $recharge_details = (int)($_POST['recharge_details'] ?? 0); // Can be 0 or a valid recharge ID
    ...
    if (empty($errors) && $user_id > 0) {
        try {
            ...
            error_log("API Cards create - User ID validation error: " . $e->getMessage());
        }
    }
    ...
    if (empty($errors) && !empty($recharge_details) && $recharge_details != 0) {
        try {
            ...
            error_log("API Cards create - Recharge ID validation error: " . $e->getMessage());
        }
    }
    ...
    if (empty($errors)) {
        try {
            ...
            $new_card_data = [
                ...
                'created_at' => date('Y-m-d H:i:s') // Set creation time
            ];

            // Attempt to create the card using the model
            $new_card_id = $cardModel->create($new_card_data);

            if ($new_card_id) {
                $response['success'] = true;
                $response['message'] = 'Card created successfully.';
                $response['data'] = ['card_id' => $new_card_id];
            } else {
                $response['message'] = 'Failed to create card. Please try again.';
            }
        } catch (PDOException $e) {
            http_response_code(500); // Internal Server Error
            $response['message'] = 'Database error during card creation.';
            error_log("API Cards create error: " . $e->getMessage());
        }
    } else {
